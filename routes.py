...
def export_corretivos_analysis(category):
    ...
    try:
        ...
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine, engine='openpyxl') as writer:
            df.to_excel(writer, sheet_name='Dados Detalhados', index=False)
...
# Define all tables before importing routes
class Revenda(db.Model):
    ...
    def get_municipios_list(self):
        ...
        import json
        try:
            # Return empty list if self.municipios is None or empty string
            return json.loads(self.municipios) if self.municipios else []
        except json.JSONDecodeError:
            print(f"Error decoding JSON for revenda ID {self.id}. Municipios data: {self.municipios}")
    ...
    def set_municipios_list(self, municipios_list):
        ...
        try:
            ...
            print(f"Error encoding list to JSON for revenda ID {self.id}: {e}. Data: {municipios_list}")
